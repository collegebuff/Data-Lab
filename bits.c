...
int conditional(int x, int y, int z) {
    ...
    mask1 = !!x; /* checks for zero or one*/
    mask2 = ~mask1 + 1;
    val1 = mask2 & y;
    val2 = ~mask2 & z;
    /* if x is zero then val1 is zero so val2 is passed in*/
    return val1 + val2;
}
/* 
...
int satAdd(int x, int y) {
    ...
    int c = y & a;
    int bc = sum & a;
    int bigflow = !!((bc^b)& (bc^c));
  return ((bigflow + neg)& sum)|((~(bigflow+neg))&(a+((~(!!bc))+1)));
}
/*
 * Extra credit
 */
